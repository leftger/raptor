    ...
    if let Some(room) = state.run.as_ref().and_then(|run| run.source_stealth()) {
        ...
        let (want_x, want_z, want_height, look) = match (room.hug, room.peek) {
            (Some(wall), Some(across)) => {
                ...
                // A corner is where the wall runs out. Along a wall that carries
                // on there is nothing to look round, so this stays close, which is
                // the plain over-the-shoulder shot down the corridor.
                ...
                let standing_out = if run < config::STEALTH_PEEK_STEPS {
                    config::STEALTH_HUG_CAMERA_OUT
                } else {
